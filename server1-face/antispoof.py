...
class FTGenerator(torch.nn.Module):
    def __init__(self, in_channels=48, out_channels=1):
        super(FTGenerator, self).__init__()
        self.ft = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels, 128, kernel_size=(3, 3), padding=1),
            torch.nn.BatchNorm2d(128),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(128, 64, kernel_size=(3, 3), padding=1),
            torch.nn.BatchNorm2d(64),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(64, out_channels, kernel_size=(3, 3), padding=1),
...
class MultiFTNet(torch.nn.Module):
    def __init__(self, model_type='MiniFASNetV2', img_channel=3, num_classes=3, embedding_size=128, conv6_kernel=(5, 5)):
        super(MultiFTNet, self).__init__()
        self.img_channel = img_channel
        self.num_classes = num_classes
        if model_type == 'MiniFASNetV1SE':
            self.model = MiniFASNetSE(keep_dict['1.8M'], embedding_size, conv6_kernel, 0.75, num_classes, img_channel)
        elif model_type == 'MiniFASNetV2':
            self.model = MiniFASNet(keep_dict['1.8M_'], embedding_size, conv6_kernel, 0.2, num_classes, img_channel)
        else:
...
# =========================================================================
# API Anti-Spoofing para Server1
# =========================================================================

MODEL_MAPPING = {
    "1": {
        "path": "antispoof_models/2.7_80x80_MiniFASNetV2.pth",
        "type": "MiniFASNetV2",
        "scale": 2.7
    },
    "2": {
        "path": "antispoof_models/4_0_0_80x80_MiniFASNetV1SE.pth",
        "type": "MiniFASNetV1SE",
        "scale": 4.0
    }
}

def cargar_modelos_antispoof(device):
    """Carga los modelos de detección de vida en la memoria de la GPU/CPU."""
    models = []
    
    # Validar que los pesos existen
    for key, model_info in MODEL_MAPPING.items():
        if not os.path.exists(model_info["path"]):
            print(f"!! -- Archivo de pesos no encontrado: {model_info['path']}")
            return None
            
    for key, model_info in MODEL_MAPPING.items():
        if model_info["type"] == 'MiniFASNetV1SE':
            model = MiniFASNetSE(keep_dict['1.8M'], 128, (5, 5), 0.75, 3, 3)
        elif model_info["type"] == 'MiniFASNetV2':
            model = MiniFASNet(keep_dict['1.8M_'], 128, (5, 5), 0.2, 3, 3)
            
        state_dict = torch.load(model_info["path"], map_location=device, weights_only=True)
        
        # Eliminar el prefijo 'module.' (debido a que se entrenó con DataParallel)
        new_state_dict = {}
        ...
        model.eval()
        models.append({"model": model, "scale": model_info["scale"]})
    return models

def get_bbox(box, scale, img_w, img_h):
    """Calcula el bounding box escalado centrado en la cara."""
    x1, y1, x2, y2 = box
    w = x2 - x1
    h = y2 - y1
    cx = x1 + w/2
    ...
    # Obtener el tamaño del crop como el máximo de W o H para mantener ratio cuadrado
    side = max(w, h) * scale
    
    new_x1 = max(0, int(cx - side/2))
    new_y1 = max(0, int(cy - side/2))
    new_x2 = min(img_w, int(cx + side/2))
    new_y2 = min(img_h, int(cy + side/2))
    
    # Convertir a cuadrado perfecto
    new_w = new_x2 - new_x1
    new_h = new_y2 - new_y1
...
def verificar_liveness(image_pil, face_box, models, device, threshold=0.80):
    """
    Recibe la imagen original, el bounding box y procesa a través de los 2 modelos de anti-spoofing.
    Retorna (is_real: bool, confidence: float)
    """
    img_np = np.array(image_pil)
    img_h, img_w, _ = img_np.shape
    
    # 2. Iterar por ambos modelos (diferentes escalas)
    prediction_scores = []
    ...
    for m in models:
        ...
        # Calcular bbox expandido
        x1, y1, x2, y2 = get_bbox(face_box, scale, img_w, img_h)
        ...
        # Redimensionar a 80x80 (input esperado por MiniFASNet)
        crop = cv2.resize(crop, (80, 80))
        
        # Convertir a BGR porque los modelos de MiniVision fueron entrenados así
        crop_bgr = cv2.cvtColor(crop, cv2.COLOR_RGB2BGR)
        
        # Pasar a tensor PyTorch [1, C, H, W]
        img_tensor = crop_bgr.transpose(2, 0, 1) # C, H, W
        ...
        with torch.no_grad():
            ...
            # Salida es un logit para 3 clases. Hacemos softmax.
            probs_out = F.softmax(out, dim=-1).cpu().numpy()[0]
            # La clase 1 es "Real face" en el modelo de minivision. 0 y 2 son spoof.
            real_score = probs_out[1]
            prediction_scores.append(real_score)
            
    if not prediction_scores:
        return False, 0.0
        
    # Promediar el score de los dos modelos (Voting)
    final_score = float(np.mean(prediction_scores))
